...
function IncreaseTemporarySavesList() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await httpRequest.get('quantrang/increase/temporarySave');
    ...
    const handleSubmit = async (rowId) => {
        ...
        try {
            const response = await httpRequest.post('quantrang/increase/temporarySave/update', {
                _id: rowId,
    ...
    const handleTransfer = async (row) => {
        try {
            const response = await httpRequest.post('quantrang/increase', { data: row });
            await httpRequest.delete('quantrang/increase/temporarySave/remove', { data: { _id: row._id } });
    ...
    return (
        <div>
            <div className={cx('temp-header')}>
                <h1 className={cx('temp-title')}>DANH SÁCH QUÂN SỐ TĂNG LƯU TẠM</h1>
...
export default IncreaseTemporarySavesList;
